...
class EventPlannerFragment : Fragment(R.layout.fragment_event_planer) {
    ...
    private val launcher = registerForActivityResult(ActivityResultContracts.GetContent()) { uri ->
        uri?.path ?: return@registerForActivityResult
        if (uri.path?.contains(".json") == true) {
            val inputStream = requireActivity().application.contentResolver.openInputStream(uri)
            viewModel.addEventsFromJson(inputStream)
        }
